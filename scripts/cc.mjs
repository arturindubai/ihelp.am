...
for (let i = 0; i < argv.length; i++) {
  ...
  if (a.startsWith("--")) {
    ...
    const next = argv[i + 1];
    if (next !== undefined && !next.startsWith("--") && !["error", "json", "force"].includes(name)) {
      flags[name] = next;
      i++;
...
async function takeTask(key) {
  ...
  const body = { action: "claim", agent, key, area: flags.area, layer: flags.layer, priority: flags.priority, session: process.env.CLAUDE_SESSION_ID };
...
✓ ${t.key} взята: ${agent}, аренда ${Math.round((new Date(t.claimUntil) - Date.now()) / 60000)} мин, пульс продлевает её сам (хук Claude Code).
  Рабочая копия: ${dir}${created ? " (создана)" : " (уже была)"}
  Ветка: ${branch}${ahead ? `\n  В ветке уже есть работа — продолжайте с неё:\n${ahead.split("\n").map((l) => `    ${l}`).join("\n")}` : ""}

Дальше:
  1. Перейдите в рабочую копию: инструмент EnterWorktree с path=${dir}
     (или работайте в ней абсолютными путями; основную копию /opt/ihelp.am не переключать).
  2. Коммиты — «${t.key}: что сделано», пуш — git push -u origin ${branch}.
  3. Ход работы и ошибки — node scripts/cc.mjs note ${t.key} "…" [--error].
  4. Сдать — node scripts/cc.mjs review ${t.key} "что сделано, как проверено, как проверить".
  Не успеваете или контекст кончается — node scripts/cc.mjs handoff ${t.key} "что сделано, что осталось".`);
}
...
async function main() {
  switch (cmd) {
    ...
    case "gc": {
      ...
      tryGit(["fetch", "-q", "origin"], ROOT);
      for (const key of fs.readdirSync(WT)) {
        const dir = path.join(WT, key);
