 ...
 *   PUT  /api/cc/library {slug, title?, content, note?, agent} (x-cc-key) — новая версия записи (cc.mjs lib update)
 */
export async function PUT(req: Request) {
  ...
  const b = (await req.json().catch(() => ({}))) as { slug?: unknown; title?: unknown; content?: unknown; note?: unknown; agent?: unknown };
  const content = typeof b.content === "string" ? b.content.trim() : "";
  if (typeof b.slug !== "string" || !b.slug) return NextResponse.json({ error: "slug_required" }, { status: 400 });
  if (content.length < 20 || content.length > 200_000) return NextResponse.json({ error: "content_required" }, { status: 400 });
  try {
    const current = await getLibraryDoc(b.slug);
    if (!current?.current) return NextResponse.json({ error: "not_found" }, { status: 404 });
    const r = await updateNote(b.slug, { title: typeof b.title === "string" && b.title ? b.title : current.current.title, content, note: typeof b.note === "string" ? b.note : undefined }, typeof b.agent === "string" && b.agent ? b.agent : "api");
    const after = await getLibraryDoc(b.slug);
    return NextResponse.json({ ok: true, changed: r.changed, version: after?.current?.n });
  } catch (e) {
