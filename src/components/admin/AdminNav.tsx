...
import { LayoutDashboard, ClipboardList, CalendarRange, Sparkles, UsersRound, Contact, Star, TicketPercent, Image, FileText, Languages, Settings, ShieldCheck, History, Menu, X, ExternalLink } from "lucide-react";
import { Link, usePathname } from "@/i18n/navigation";
import { cn } from "@/lib/format";

const ICONS = { dashboard: LayoutDashboard, orders: ClipboardList, schedule: CalendarRange, services: Sparkles, masters: UsersRound, clients: Contact, reviews: Star, promos: TicketPercent, banners: Image, pages: FileText, translations: Languages, settings: Settings, staff: ShieldCheck, log: History };
...
export function AdminNav({ sections, brand }: { sections: string[]; brand: string }) {
  ...
  const list = (
    <nav className="space-y-0.5">
      {items.map((i) => (
        <Link key={i.key} href={i.href} onClick={() => setOpen(false)} className={cn("flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium", isActive(i.href) ? "bg-ink text-white" : "text-ink hover:bg-surface")}>
  ...
  return (
    <>
      <aside className="sticky top-0 hidden h-dvh w-60 shrink-0 overflow-y-auto border-r border-line bg-white p-3 md:block">
        <div className="mb-4 flex items-center gap-2 px-2 pt-1 font-bold"><img src="/img/icon.svg" alt="" className="size-7 rounded-lg" /> {brand}</div>
        {list}
      </aside>
      ...
      {open && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="relative h-full w-72 overflow-y-auto bg-white p-3">
