...
Прототип — Command Center проекта LIA. Оттуда взяты: доска как единственный источник правды; роли с чёткими границами; аренда задачи с пульсом; сторож для брошенных задач; «Сделано» только с доказательством; эпик закрывается только после всех своих задач; возврат на доработку с причиной; карточка, где по клику видно всё. Не взяты, по решению владельца от 23.09.2026: журнал задач в `.git`, хуки, которые блокируют правки, и агенты, которые сами мёрджат и выкладывают.

---

## 1. Восемь правил

1. **Control Center — единственный источник правды.** Нет карточки — нет работы. Любая работа начинается с карточки и заканчивается отчётом в ней. Ветку без карточки деплоер не рассматривает. Новая работа приходит через Intake: кнопка «✦ Задача» в Control Center или `node scripts/cc.mjs intake "…"` из любого чата. Чат, которого попросили о новой работе, кладёт её в очередь, а не делает сам.
...
3. **«Сделано» — только с доказательством.** Для код-задачи это коммит в `main` и описание, что проверено после выкладки. «Собралось» и «тесты зелёные» — ещё не «готово».
...
| Триаж | один воркер Claude | каждая новая карточка бэклога (и входящие из Intake) — проверка готовности, дополнение полей, в очередь или вопрос владельцу; раз в сутки — обзор всего бэклога | [roles/TRIAGE.md](roles/TRIAGE.md) |
...
| Продукт и не-код | воркер Claude | задачи без кода: исследование, инструкции, тексты, настройка внешних сервисов; результат — в карточке, приёмка — владельцем в «Согласованиях» | [roles/NOCODE.md](roles/NOCODE.md) |
...
Первые сообщения для каждого чата — в [roles/KICKOFF.md](roles/KICKOFF.md). Триаж, разработчиков, тестировщика и деплоера можно не запускать руками: их по очереди задач запускает диспетчер — [WORKERS.md](WORKERS.md). Управление и очереди — Control Center → «Воркеры».
...
| Проверить готовность, перевести в «В очереди» | ✅ | ✅ | ✅ | — | — | — | — |
...
| Проверить, выложить, закрыть «Сделано» | ✅ | — | принимает продуктовые | принимает вид | — | ✅ | — |
...
|---|---|---|---|
| **Бэклог** | идея или потребность, к работе ещё не готова; новая карточка ждёт триажа | любой, кто завёл задачу; из свободного текста — кнопка «✦ Задача» (Intake) | — |
| **В очереди** | чек-лист готовности пройден, задача ждёт исполнителя | триаж, CTO, продакт, владелец | обязательные пункты готовности (раздел 4) |
| **В работе** | задачу держит исполнитель с живой арендой | только сам исполнитель командой `take` или `next` | задача в статусе «В очереди», зависимости закрыты, исполнитель свободен, её файлы не меняет другая задача в работе |
| **На проверке** | ветка отправлена, отчёт написан; очередь деплоера | исполнитель | у код-задачи — ветка, у любой — отчёт от 40 символов |
| **Сделано** | влито в `main`, выложено, проверено | деплоер или владелец | у код-задачи — коммит в `main` и что проверено; у прочих — доказательство словами или файлом |
...
Причину словами нужно написать при возврате на доработку, передаче, блокировке, отмене, переоткрытии и возврате из «В очереди» в бэклог.

Обойти гейт могут только владелец и CTO, и всегда с причиной. В истории это отмечается строкой «Проверка обойдена».

Эпик получает статус «Сделано», только когда все его задачи закрыты: либо «Сделано», либо «Отменена».
...
| Поле | Зачем | Нужно для «В очереди» |
|---|---|---|
...
| Нужен макет (`mockupRequired`) | триаж ставит для новых экранов и заметных изменений; снимается только утверждением владельца | **обязательно**: задача не уходит в «В очереди», пока владелец не утвердит макет (гейт `mockup_required`) |
...
Чек-лист готовности виден в карточке задачи. Жёсткие пункты («зачем» и хотя бы один критерий) не пускают задачу в «В очереди». Остальные пункты — предупреждения: CTO смотрит их прежде, чем отдать задачу разработчику.
...
- **Сторож** запускается каждые 15 минут из `/api/cron`. Он:
  ...
  - через 4 часа после метки возвращает задачу туда, откуда её взяли — обычно в «В очереди». Ветка сохраняется, счётчик «бросали» растёт, в ленту попадает последняя запись исполнителя;
...
- Не годится → `node scripts/cc.mjs return КЛЮЧ "что исправить" --agent deployer` (тестировщик — `fail`). Задача уходит в «В очереди» с меткой «возвращалась», следующий исполнитель берёт её первой и продолжает с той же ветки.
- Годится → `scripts/deploy-task.sh КЛЮЧ`, единственный путь в прод и для воркера, и для чата. Скрипт делает слияние, бэкап перед миграцией, `deploy/update.sh` и smoke-тест. При провале он откатывает прод и возвращает задачу, при успехе закрывает её «Сделано» с коммитом и доказательством.
...
- новый экран или заметная смена вида — нужно утверждение макета (`cc.mjs mockup КЛЮЧ "комментарий"` или кнопка в карточке);
...
|---|---|---|
| Статус «В работе» ставился из кода бэклога | 13 задач числились «в работе», хотя никто над ними не работал | из кода задача получает только «Сделано» или «На проверке» (если заведена в той же ветке, что и её код); «В работе» даёт только аренда; сторож сообщает о задачах «без исполнителя» |
...
| «Сделано» без проверки в проде (опыт LIA: ложные «Done») | в продукте оказывались заглушки | «Сделано» требует коммит и доказательство |
...
- **DEV-17** — Control Center по образцу LIA: вкладки, очереди воркеров, Intake, авто-триаж, релизы, здоровье, логи; [CONTROL_CENTER.md](CONTROL_CENTER.md), [roles/TRIAGE.md](roles/TRIAGE.md). Страница «Логи» закрывает часть DEV-14 (ошибки приложения без SSH).
