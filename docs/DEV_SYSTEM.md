...
| Разработчик | чаты Claude, от одного до нескольких параллельно | одна задача за раз: код, тесты, ветка, отчёт | [roles/DEVELOPER.md](roles/DEVELOPER.md) |
| Деплоер | один чат Claude | проверка ветки, стенд, мёрдж, выкладка, закрытие с доказательством, возврат на доработку | [DEPLOYER_GUIDE.md](DEPLOYER_GUIDE.md) |
...
Первые сообщения для каждого чата — в [roles/KICKOFF.md](roles/KICKOFF.md).
...
- Очередь деплоера — статус «На проверке»: `node scripts/cc.mjs list review`. Отчёт и факты из git — в ленте задачи.
- Не годится → `node scripts/cc.mjs return КЛЮЧ "что исправить" --agent deployer`. Задача уходит в «Готова к работе» с меткой «возвращалась», следующий исполнитель берёт её первой и продолжает с той же ветки.
- Годится → мёрдж, бэкап перед миграцией, `deploy/update.sh`, smoke-тест → `node scripts/cc.mjs done КЛЮЧ --sha <коммит в main> "что проверено после выкладки" --agent deployer`.
...
- **DEV-6** — разработчики по расписанию. Всё, кроме запуска, уже готово; нужно решение владельца по бюджету и доступам.
